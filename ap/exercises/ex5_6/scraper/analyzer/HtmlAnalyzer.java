...
public class HtmlAnalyzer {
    ...
    public static void printTopCountUrls(int k) {
        ...
        MyObjectCounter urlsCounter = new MyObjectCounter();
        getAllUrls().forEach(s -> urlsCounter.add(s));
        String[][] top = urlsCounter.getTop(k);
        for (int i = 0; i < k; i++)
            System.out.println(top[i][0] + " -> " + top[i][1]);
    }
